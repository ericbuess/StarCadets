...
import { createRacerState, updateRacer, renderRacer } from "@/game/polePositionEngine"
import type { RacerState, RacerInput } from "@/game/polePositionEngine"
...
export function PolePositionGame({ subjects, grade, testMode = false, onToggleTestMode, onGameOver, onBackToHub }: Props) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const stateRef = useRef<RacerState | null>(null)
  const inputRef = useRef<RacerInput>({ left: false, right: false, up: false, down: false })
  ...
  const shadow = useShadowSession("poleposition")
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      if (!showEducation) {
        const newState = updateRacer(state, dt, inputRef.current)
      ...
      renderRacer(ctx, state, canvas.width, canvas.height)
  ...
  const handleTap = useCallback(() => {
    ...
    if (!state) return
    if (state.gameOver) {
      if (testMode) { stateRef.current = createRacerState(); lastTimeRef.current = 0; shadow.restart() }
  ...
  const handleEducationComplete = useCallback((passed: boolean) => {
    ...
    if (passed) {
      ...
      onGameOver?.(Math.floor(state.score), Math.floor(state.position))
    }
